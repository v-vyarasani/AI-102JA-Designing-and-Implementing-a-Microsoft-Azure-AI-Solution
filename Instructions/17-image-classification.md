...
3. Custom Vision ポータルで、次の設定で新しいプロジェクトを作成します。
    ...
    - **ドメイン**: Food
4. 新しいプロジェクトで、**「Add images」** をクリックし、前に表示した **training-images/apple** フォルダー内のすべてのファイルを選択します。次に、次のようにタグ *apple* を指定して、画像ファイルをアップロードします
...
7. Custom Vision プロジェクトでは、画像の上にある **「Train」 **をクリックしげ、タグ付けされた画像を使用して分類モデルをトレーニングします。**「Quick Training」** オプションを選択し、トレーニングの反復が完了するのを待ちます (これには 1 分ほどかかる場合があります)。
...
2. **「Image URL」** ボックスに`https://aka.ms/apple-image`と入力し、予測を行います。
3. モデルによって返される予測を表示します。次のように、*apple*の確率スコアが最も高くなります。
...
2. **「General」**の下で、このプロジェクトを一意に識別する **「Project ID」** に注意してください。
3. 右側の **「Resource」** の下に、キーとエンドポイントを含む*トレーニング* リソースの詳細が表示されていることに注意してください (この情報は、Azure portal でリソースを表示することでも取得できます)。
